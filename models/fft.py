...
def _create_vision_transformer(variant='vit_base_patch16_224.augreg_in21k_ft_in1k', pretrained=False, **kwargs):
    ...
    model = QuantViT()# CPEViT() # QuantizeViT() # VisionTransformer() # ViT_lora_fft()
    ...
    print('Missing keys when loading pretrained weights: {}'.format(missing_keys))
    print('Unexpected keys when loading pretrained weights: {}'.format(unexpected_keys))
...
class SiNet(nn.Module):
    ...
    def forward(self, image, get_feat=False, get_cur_feat=False, fc_only=False, labels=None):
        if fc_only:
            fc_outs = []
        ...
        logits = []
        image_features, prompt_loss = self.image_encoder(image, task_id=self.numtask-1, get_feat=get_feat, get_cur_feat=get_cur_feat, labels=labels)
        image_features = image_features[:,0,:]
